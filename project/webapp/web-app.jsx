...
(function () {
    ...
    return (
      <Modal t={t} open={open} onClose={onClose} width={440}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <h3 style={{ margin: 0, fontSize: 18, fontWeight: 720, color: t.text }}>补充重复范围</h3>
        </div>
    ...
    return (
      <div style={{ width: 248, flexShrink: 0, background: t.surface, borderRight: `1px solid ${t.border}`, display: 'flex', flexDirection: 'column', padding: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '4px 6px 18px' }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 760, color: t.text, letterSpacing: -0.3 }}>语迹 VoiceLog</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {nav.map(([k, lab, ic]) => {
            const on = tab === k;
            return (
              <button key={k} onClick={() => setTab(k)} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 12px', borderRadius: t.radius - 2, border: 'none', cursor: 'pointer', font: 'inherit', fontSize: 15, fontWeight: on ? 650 : 500, color: on ? t.accentText : t.muted, background: on ? t.accentSoft : 'transparent', textAlign: 'left' }}>
        ...
        {level && (
          <button onClick={onGrowth} title="成长" style={{ display: 'block', width: '100%', textAlign: 'left', cursor: 'pointer', font: 'inherit', padding: 12, marginBottom: 10, borderRadius: t.radius, border: `1px solid color-mix(in oklch, ${GOLD} 40%, ${t.border})`, background: t.mode === 'dark' ? 'linear-gradient(150deg,#16302c,#102420)' : `linear-gradient(150deg, color-mix(in oklch, ${GOLD} 13%, ${t.surface}), ${t.surface})` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 30, height: 30, borderRadius: 999, flexShrink: 0, border: `1.5px solid ${GOLD}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13.5, fontWeight: 720, color: GOLD }}>{level.lv}</span>
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 650, color: t.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{level.name}</div><div style={{ fontSize: 11.5, color: t.faint }}>LV.{level.lv} · {xp} XP</div></div>
            </div>
        ...
        <div style={{ padding: 12, borderRadius: t.radius, background: t.surface2 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Dot color={aiEngine ? 'oklch(0.62 0.15 150)' : 'oklch(0.70 0.14 70)'} size={9} ring /><span style={{ fontSize: 13.5, fontWeight: 600, color: t.text }}>{aiEngine ? 'AI 解析' : '规则解析'}</span></div>
    ...
    return (
      <div style={{ width: 312, flexShrink: 0, borderLeft: `1px solid ${t.border}`, background: t.surface, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 18 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: t.text, marginBottom: 12 }}>{w.today ? '今天' : `周${w.dow}`} · {w.month}月{w.day}日 <span style={{ fontSize: 13.5, fontWeight: 500, color: t.faint }}>{list.length} 项</span></div>
          {list.length ? <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(() => { const grp = (e) => { const m = window.VL.timeMode(e); return m === 'allday' ? 'allday' : m === 'untimed' ? 'untimed' : 'timed'; }; const HEAD = { allday: '全天 · 贯穿一整天', timed: '具体时间', untimed: '随手记 · 没有具体时间' }; const mixed = new Set(list.map(grp)).size > 1; const items = []; let last = null; list.forEach((e) => { const g = grp(e); if (mixed && g !== last) { items.push({ _hd: g, _label: HEAD[g] }); last = g; } items.push(e); }); return items; })().map((ev) => {
              if (ev._hd) return <div key={'h-' + ev._hd} style={{ fontSize: 11.5, fontWeight: 700, color: t.faint, letterSpacing: 0.5, marginTop: 2 }}>{ev._label}</div>;
                ...
                <div key={ev.id} onClick={() => app.openDetail(ev)} style={{ display: 'flex', gap: 10, padding: 11, borderRadius: t.radius - 2, border: `1px solid ${t.border}`, background: t.bg, cursor: 'pointer', opacity: cancelled ? 0.6 : 1 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 3 }}>
                      ...
                      {conf && <span onClick={(e) => { e.stopPropagation(); app.showMultitask(); }} style={{ fontSize: 11.5, fontWeight: 600, color: 'oklch(0.58 0.15 60)', background: 'color-mix(in oklch, oklch(0.72 0.15 70) 16%, transparent)', padding: '1px 7px', borderRadius: 999 }}>重叠</span>}
                    </div>
                    {ev.progress && <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 7 }}><div style={{ flex: 1, height: 5, borderRadius: 999, background: t.chartTrack, overflow: 'hidden' }}><div style={{ height: '100%', width: `${ev.progress.done / ev.progress.total * 100}%`, background: catColor(t, ev.cat), borderRadius: 999 }} /></div><span style={{ fontSize: 11.5, fontWeight: 600, color: t.faint, fontVariantNumeric: 'tabular-nums' }}>{ev.progress.done}/{ev.progress.total}</span></div>}
                  </div>
          ...
          </div> : <div style={{ textAlign: 'center', padding: '30px 0', color: t.faint, fontSize: 13.5 }}>这天还没有安排</div>}
        </div>
      </div>
    );
  }
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          {app.hasPending && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 24px', background: t.accentSoft, borderBottom: `1px solid ${t.border}` }}>
              ...
              <div style={{ flex: 1, fontSize: 13.5, color: t.accentText, lineHeight: 1.5 }}>课表已加入，有 <b>{app.pendingCount}</b> 节课的重复范围先按本学期暂存。知道结课日期了就补上，每周自动重复到那天。</div>
              <button onClick={app.openRecur} style={{ flexShrink: 0, height: 32, padding: '0 14px', borderRadius: t.radius - 4, border: 'none', cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 650, background: t.accent, color: t.onAccent }}>补充范围</button>
            </div>
          )}
          {view === 'matrix'
  ...
  function WebReflectBlock({ t, app }) {
    ...
    return (
      <Card t={t}>
        ...
        <textarea value={text} onChange={(e) => setText(e.target.value)} onBlur={save} rows={4} placeholder="今天过得怎么样？随便写写，或点「说一段」用语音记。" style={{ width: '100%', resize: 'none', border: 'none', outline: 'none', background: 'transparent', color: t.text, font: 'inherit', fontSize: 15, lineHeight: 1.6 }} />
          ...
          <div style={{ width: 320, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <Card t={t}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '14px 0' }}>
                <div><div style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}><span style={{ fontSize: 44, fontWeight: 780, color: t.text, letterSpacing: -1, lineHeight: 1 }}>{fmtH(r.total)}</span><span style={{ fontSize: 15, color: t.muted, fontWeight: 600 }}>小时</span></div><div style={{ fontSize: 13.5, color: t.faint, marginTop: 6 }}>共 {r.count} 项日程</div></div>
                <Ring t={t} value={r.rate} size={96} />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>{[['完成', r.done], ['取消', r.cancelled], ['待办', r.todo]].map(([k, v]) => <div key={k} style={{ flex: 1, padding: '10px 0', borderRadius: t.radius - 4, background: t.surface2, textAlign: 'center' }}><div style={{ fontSize: 18, fontWeight: 720, color: t.text }}>{v}</div><div style={{ fontSize: 11.5, color: t.faint, marginTop: 1 }}>{k}</div></div>)}</div>
            </Card>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button onClick={() => app.setTab('growth')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, height: 32, padding: '0 12px 0 9px', borderRadius: 999, cursor: 'pointer', border: `1px solid ${t.border}`, background: t.surface, color: t.muted, font: 'inherit', fontSize: 13.5, fontWeight: 600 }}><Icon name="chevL" size={15} color={t.muted} sw={2.2} />返回</button>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          <div style={{ width: 320, flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
              {F.map((f) => { const on = fmt === f.key; return (
                <button key={f.key} onClick={() => setFmt(f.key)} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: 14, cursor: 'pointer', borderRadius: t.radius, font: 'inherit', textAlign: 'left', background: on ? t.accentSoft : t.surface, border: `1.5px solid ${on ? t.accentText : t.border}`, boxShadow: on ? 'none' : t.shadow }}>
                  ...
                  <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 650, color: t.text }}>{f.name}</div><div style={{ fontSize: 12.5, color: t.muted, marginTop: 2 }}>{f.desc}</div></div>
                </button>
              ); })}
            </div>
            ...
            <div style={{ display: 'flex', gap: 7, alignItems: 'center', justifyContent: 'center', marginTop: 14 }}><Icon name="shield" size={14} color={t.faint} /><span style={{ fontSize: 12.5, color: t.faint }}>保存在本机 · 不经过云端</span></div>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
  ...
  function SettingsView({ t, app, baseKey }) {
    ...
    const Row = ({ icon, title, sub, right, last }) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '13px 14px', borderBottom: last ? 'none' : `1px solid ${t.border}` }}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 600, color: t.text }}>{title}</div>{sub && <div style={{ fontSize: 12.5, color: t.faint, marginTop: 1 }}>{sub}</div>}</div>{right}
      </div>
    );
    ...
    return (
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', maxWidth: 720 }}>
        ...
        <div style={{ display: 'flex', gap: 13, padding: 16, borderRadius: t.radius, marginBottom: 20, background: t.accentSoft }}><Icon name="shield" size={22} color={t.accentText} style={{ flexShrink: 0, marginTop: 1 }} /><div><div style={{ fontSize: 15, fontWeight: 700, color: t.accentText }}>本地优先</div><div style={{ fontSize: 13.5, color: t.accentText, opacity: 0.85, marginTop: 3, lineHeight: 1.5 }}>所有日程与复盘都存在你自己的机器上，核心功能离线即可运行，不依赖任何云服务。</div></div></div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
            {Object.values(window.VL.themes).map((th) => { const on = baseKey === th.key; return (
              <button key={th.key} onClick={() => app.setBase(th.key)} style={{ flex: 1, padding: '12px 0', borderRadius: t.radius - 2, cursor: 'pointer', font: 'inherit', border: `1.5px solid ${on ? t.accentText : t.border}`, background: on ? t.accentSoft : t.surface, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
                <span style={{ width: 30, height: 30, borderRadius: 9, background: th.accent }} /><span style={{ fontSize: 13.5, fontWeight: on ? 650 : 500, color: on ? t.text : t.muted }}>{th.name} · {th.en}</span>
              </button>
            ); })}
          </div>
        ...
        {app.setTimeFmt && (<React.Fragment>
          ...
          <Card t={t} style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: t.text }}>时间制式</div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[['24', '24 小时'], ['12', '12 小时']].map(([k, label]) => { const on = app.timeFmt === k; return (
                  <button key={k} onClick={() => app.setTimeFmt(k)} style={{ height: 34, padding: '0 15px', borderRadius: 999, cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 600, border: `1px solid ${on ? 'transparent' : t.border}`, background: on ? t.accentSoft : t.surface, color: on ? t.accentText : t.muted }}>{label}</button>
                ); })}
              </div>
            </div>
          </Card>
        ...
        <Card t={t} pad={0} style={{ marginBottom: 16, overflow: 'hidden' }}>
          ...
          <Row icon="trash" title={`回收站 · ${app.trash.length} 项`} sub="删除的日程先放这里，随时找回" right={<button onClick={app.openTrash} style={{ height: 32, padding: '0 14px', borderRadius: 999, border: `1px solid ${t.border}`, cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 600, background: t.surface, color: t.text }}>打开</button>} />
        ...
        <Card t={t} pad={0} style={{ marginBottom: 16, overflow: 'hidden' }}>
          <Row icon="export" title="备份到文件" sub="把全部日程下载成一个 .json 文件，换设备、清缓存都不怕" right={<button onClick={app.backup} style={{ height: 32, padding: '0 14px', borderRadius: 999, border: 'none', cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 600, background: t.accent, color: t.accentText }}>备份</button>} />
          <Row icon="redo" title="从备份恢复" sub="选择之前的 .json 备份覆盖恢复（会先确认）" right={<button onClick={() => fileRef.current && fileRef.current.click()} style={{ height: 32, padding: '0 14px', borderRadius: 999, border: `1px solid ${t.border}`, cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 600, background: t.surface, color: t.text }}>恢复</button>} last />
        </Card>
        ...
        <Card t={t} pad={0} style={{ marginBottom: 16, overflow: 'hidden' }}>
          {app.demoMode
            ? <Row icon="redo" title="退出示例预览" sub="回到你自己的数据（你的内容一直安全保留）" right={<button onClick={app.exitDemo} style={{ height: 32, padding: '0 14px', borderRadius: 999, border: `1px solid ${t.border}`, cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 600, background: t.surface, color: t.text }}>退出</button>} last />
            : <Row icon="sparkle" title="查看示例数据" sub="载入一份样例感受完整效果，随时可一键退出" right={<button onClick={app.enterDemo} style={{ height: 32, padding: '0 14px', borderRadius: 999, border: 'none', cursor: 'pointer', font: 'inherit', fontSize: 13.5, fontWeight: 600, background: t.accent, color: t.accentText }}>查看</button>} last />}
        </Card>
      </div>
    );
  }
  ...
  function DetailModal({ t, app }) {
    ...
    const meta = (icon, label, val) => <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: `1px solid ${t.border}` }}><Icon name={icon} size={17} color={t.faint} /><span style={{ fontSize: 13.5, color: t.muted, width: 52 }}>{label}</span><span style={{ fontSize: 15, color: t.text, fontWeight: 550 }}>{val}</span></div>;
    return (
      <Modal t={t} open={!!ev} onClose={() => app.setDetail(null)}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}><h3 style={{ margin: 0, fontSize: 22, fontWeight: 720, color: t.text }}>{ev.title}</h3><div style={{ marginTop: 6, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}><Chip t={t} color={col} soft><Dot color={col} size={7} />{catLabel(t, ev.cat)}</Chip><button onClick={() => app.showMatrix()} style={{ border: 'none', background: 'transparent', cursor: 'pointer', padding: 0, font: 'inherit' }} title="四象限"><window.QuadrantChip t={t} ev={ev} /></button></div></div>
        ...
        <div style={{ marginTop: 12 }}>{meta('clock', '时间', window.VL.timeMode(ev) === 'at' ? `${window.VL.fmtRange(ev.t, ev.dur)} · ${ev.dur} 分钟` : (window.VL.timeMode(ev) === 'allday' ? '全天 · 贯穿一整天' : (window.VL.timeLabel(ev) || '随手记 · 未设时间')))}{ev.repeat && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: `1px solid ${t.border}` }}>
            ...
            <span style={{ fontSize: 13.5, color: t.muted, width: 52 }}>重复</span>
            <span style={{ fontSize: 15, color: t.text, fontWeight: 550 }}>每周{['日', '一', '二', '三', '四', '五', '六'][ev.repeat.dow]} · {ev.repeat.until ? `至 ${ev.repeat.untilText}` : '范围待补充'}</span>
        ...
        {ev.note && <div style={{ marginTop: 14, padding: 14, borderRadius: t.radius - 2, background: t.surface2 }}><div style={{ fontSize: 12.5, color: t.faint, fontWeight: 600, marginBottom: 5 }}>备注</div><div style={{ fontSize: 15, color: t.text, lineHeight: 1.55 }}>{ev.note}</div></div>}
        {ev.progress && (
          <div style={{ marginTop: 14, padding: 14, borderRadius: t.radius - 2, background: t.surface2 }}>
            ...
            <div style={{ fontSize: 12.5, color: t.muted, lineHeight: 1.55, marginTop: 10 }}>长期任务不必一次做完。<b style={{ color: t.text }}>开始了就好</b>，每推进一步都算数。</div>
  ...
  function EditModal({ t, app }) {
    ...
    const ed = (ref, val) => <span ref={ref} contentEditable suppressContentEditableWarning style={{ fontSize: 15, fontWeight: 600, color: t.text, outline: 'none', borderRadius: 6, padding: '7px 11px', background: t.surface2, display: 'inline-block', minWidth: 160 }}>{val}</span>;
    ...
    return (
      <Modal t={t} open={!!ev} onClose={() => app.setEditEv(null)}>
        <h3 style={{ margin: '0 0 16px', fontSize: 22, fontWeight: 720, color: t.text }}>编辑日程</h3>
        ...
        {row('备注 / 子任务', <span ref={noteRef} contentEditable suppressContentEditableWarning style={{ display: 'block', fontSize: 15, fontWeight: 500, color: t.text, outline: 'none', borderRadius: 8, padding: '10px 12px', background: t.surface2, minHeight: 54, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{ev.note || ''}</span>)}
    ...
    return (
      <div style={{ position: 'absolute', inset: 0, background: t.bg, color: t.text, fontFamily: t.font, display: 'flex', overflow: 'hidden', WebkitFontSmoothing: 'antialiased' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          {demoMode && (
            <div onClick={app.exitDemo} style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '8px 14px', background: t.accent, color: t.onAccent, cursor: 'pointer', fontSize: 13.5, fontWeight: 700 }}>
            ...
            return (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <h3 style={{ margin: 0, fontSize: 18, fontWeight: 720, color: t.text }}>待执行清单 · 找到 {batch.length} 条</h3>
                </div>
                <p style={{ margin: '0 0 14px', fontSize: 13.5, lineHeight: 1.6, color: t.muted }}>一段话里识别出多条意图。核对一下，去掉不要的，确认后一起执行——不会替你静默操作。</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
              {trash.map((it) => { const wk = window.VL.data.week.find((x) => x.key === it.day); return (
                <div key={it.ev.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: 11, borderRadius: t.radius - 2, border: `1px solid ${t.border}`, background: t.bg }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 600, color: t.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.ev.title}</div><div style={{ fontSize: 12.5, color: t.faint, marginTop: 2 }}>{wk ? `周${wk.dow} · ${wk.month}月${wk.day}日` : it.day} · {window.VL.timeLabel(it.ev) || '随手记'}</div></div>
