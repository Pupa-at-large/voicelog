...
(function () {
  ...
  const {
    Icon, Sheet, Btn, HomeScreen, ReviewScreen, ExportScreen, MeScreen,
  ...
  function VoiceLogApp({ theme }) {
    ...
    useEffect(() => {
      try { localStorage.setItem(SKEY(theme.key), JSON.stringify({ events, accentKey })); } catch (e) {}
    }, [events, accentKey]);
    ...
    const app = {
      ...
      toggleDone: (id) => mutate(selectedDay, (arr) => arr.map((e) =>
    ...
    return (
      <div style={{
        ...
        <TabBar t={t} tab={tab} setTab={setTab} onMic={() => setVoiceOpen(true)} />
