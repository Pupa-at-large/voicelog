...
时间精度 timeMode：用户说了具体几点(如"三点""15:00")→"at"，time 填 HH:MM；只说了大概时段(上午/中午/下午/晚上/凌晨)而没具体点→"period"，daypart 填 morning|noon|afternoon|evening|dawn|night，time 留空；说"全天/一整天/一直"→"allday"；只是记一笔流水账、完全没提时间→"untimed"。非"at"时不要编造具体钟点。
补录：若用户在记录"已经发生的事"（过去式"报名了/确定了/做了/去了"，或"今早/刚才"），把 done 设为 true，date/time 用实际发生的时间（默认今天；多件按先后顺序排早上的时间）。
做了多久：若说了时长（"自习了两个小时""写了半小时"），把时长换算成分钟填 dur（两个小时=120、半小时=30），并从 title 里去掉时长词——例"下午自习了两个小时"→ title"自习"、timeMode"period"、daypart"afternoon"、dur 120。注意"两小时前"是时间不是时长。
...
输出：{"actions":[{"kind":"create|complete|reschedule|cancel","targetId":null,"title":"","date":"YYYY-MM-DD","time":"HH:MM","timeMode":"at|period|allday|untimed","daypart":null,"dur":60,"cat":"meet|deep|life|learn|misc","loc":null,"reminder":0,"important":false,"urgent":false,"done":false,"subtasks":[]}],"cleaned":""}`;
